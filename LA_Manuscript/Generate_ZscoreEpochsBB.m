...
for mmi = 1:height(mDir2)           % -------------------- 1
    ...
    for tti = 1:height(allFoooftab) % -------------------- 2
        ...
        if matches(searchName,bDir3)
            ...
            sdIBI_spFIXED = 1.4826 * mad(tmpBlMatSP_FIXED, 1, 1);
            ...
            % 3. JATspecP KNEE
            muIBI_spKNEE = median(tmpBlMatSP_KNEE, 1, 'omitnan');
            sdIBI_spKNEE = 1.4826 * mad(tmpBlMatSP_KNEE, 1, 1);
            ...
            % 3. JAT SpecP Knee Fit parameters
            % Exponent
            muIBI_Exp_KNEE = median(tmpBl_Exp_KNEE);
            sdIBI_Exp_KNEE = mad(tmpBl_Exp_KNEE,1); % adds scaling to MAD

            epoch_Exp_KNEE = tmpROW.SpecPARAMJ{1}.KNEE.params.exponent;
            zEpoch_Exp_KNEE = (epoch_Exp_KNEE - muIBI_Exp_KNEE) ./sdIBI_Exp_KNEE;

            % Offset
            muIBI_Off_KNEE = median(tmpBl_Off_KNEE);
            sdIBI_Off_KNEE = mad(tmpBl_Off_KNEE,1); % adds scaling to MAD

            epoch_Off_KNEE = tmpROW.SpecPARAMJ{1}.KNEE.params.offset;
            zEpoch_Off_KNEE = (epoch_Off_KNEE - muIBI_Off_KNEE) ./sdIBI_Off_KNEE;

            % Ouput table
            zEpoch_params_KNEE = table(zEpoch_Exp_KNEE,zEpoch_Off_KNEE,'VariableNames',{'Z_Exp','Z_Off'});
            ...
            allFoooftab.SpecPARAMJ{tti}.KNEE.AfitParms = zEpoch_params_KNEE;
