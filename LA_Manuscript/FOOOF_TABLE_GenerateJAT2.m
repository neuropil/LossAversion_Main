function [allFoooftab] = FOOOF_TABLE_GenerateJAT2(subjectID)
...
for i = 1:numel(dirEpochNames)
    ...
    trialID = reshape(repmat(1:135,3,1),405,1);
    blockID = reshape(repmat(1:5,81,1),405,1);
    ...
    for eii = 1:height(EpochEphysTab)
            ...
            if length(tempEphys(1,:)) < 499
                ...
                specParm_results = jatSpecPARAM(tempEphys);
                FOOOF_tab.FOOOFoutput{eii} = fooofRes;
                FOOOF_tab.RAWlfp{eii} = tempEphys;
                FOOOF_tab.SpecPARAMJ{eii} = specParm_results;
                FOOOF_tab.TrialID(eii) = trialID(eii);
                FOOOF_tab.BlockID(eii) = blockID(eii);
            end
    end
