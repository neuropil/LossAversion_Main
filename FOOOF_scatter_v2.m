...
for i = 1:length(dirEpochNames)
    ...
    % Combine outcome 
    allOutcomePower = [outGGPower; outGLPower; outANPower];

    % Create labels for box plots 
    choicePowLab = repmat({'Choice'}, size(choicePower));
    respPowLab = repmat({'Response'}, size(respPower));
    OutGGPowLab = repmat({'OutGG'}, size(outGGPower));
    OutGLPowLab = repmat({'OutGL'}, size(outGLPower));
    OutANPowLab = repmat({'OutAN'}, size(outANPower));
    OutALLPowLab = repmat({'Outcome'}, size(allOutcomePower));
    
    tiledlayout(1,2)
    nexttile
    boxchart(categorical([choicePowLab; respPowLab; OutALLPowLab]), ...
        [choicePower; respPower; allOutcomePower])
    nexttile 
    boxchart(categorical([OutGGPowLab; OutGLPowLab; OutANPowLab]), ...
        [outGGPower; outGLPower; outANPower])
    % hold on 
    title(tempNamePlot)
    ...
    saveas(figure(1), [tempNamePlot + "-boxplots.jpg"]);
