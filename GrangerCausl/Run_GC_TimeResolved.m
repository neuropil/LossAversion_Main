...
%   'commonTrials' [false]  restrict every event to the trials usable in ALL
%                      events. Default false uses each event's own full set,
%                      which gives the best per-event estimate but means the
%                      events are NOT trial-matched -- so an apparent
%                      difference between two event traces could in principle
%                      come from the trials differing rather than the timing.
%                      Set true when the cross-event contrast is the point.
...
ip.addParameter('commonTrials',false,@(x)islogical(x)||isnumeric(x));
...
for e = 1:numel(o.events)
    ...
    if V
        fprintf('  %-12s %d/%d trials usable\n', o.events{e}, ...
                sum(VALe{e}), numel(VALe{e}));
    end
end
if o.commonTrials
    nT = min(cellfun(@numel, VALe));
    keep = true(nT,1);
    for e = 1:numel(o.events), keep = keep & VALe{e}(1:nT); end
    for e = 1:numel(o.events), EP{e} = ALLe{e}(keep); end
    if V
        fprintf('  commonTrials ON -> %d trials shared by all %d events\n', ...
                sum(keep), numel(o.events));
    end
else
    for e = 1:numel(o.events), EP{e} = ALLe{e}(VALe{e}); end
    nn = cellfun(@numel, EP);
    if V && numel(unique(nn)) > 1
        fprintf(['  NOTE: trial counts differ across events (%s). Traces are\n' ...
                 '        not trial-matched; pass ''commonTrials'',true if the\n' ...
                 '        cross-event comparison matters.\n'], mat2str(nn(:)'));
    end
end
nodeLabel = info.nodeLabel;
nChan = numel(nodeLabel);
empty = find(cellfun(@isempty, EP));
if ~isempty(empty)
    error('Run_GC_TimeResolved:noTrials', ...
        'Event(s) %s have no usable trials. Check epochOpts match your pipeline run.', ...
        strjoin(o.events(empty), ', '));
end
nSamp = min(cellfun(@(c) min(cellfun(@(x) size(x,2), c)), EP));
if nSamp < o.winLen
    error('Run_GC_TimeResolved:winLen', ...
        'winLen %d exceeds the epoch length %d.', o.winLen, nSamp);
end
fsA   = info.fsAnalysis;
...
TR.commonTrials = o.commonTrials;
