...
%   'shareY'  [true]  put every panel of 'band'/'top' on ONE y scale, so panel
%                     heights are directly comparable. Set false for per-panel
%                     autoscaling when one edge dwarfs the rest.
%   'ylim'    []      explicit [lo hi], overrides shareY.
%   'saveFig' ''
%   'visible' ['on']
...
ip.addParameter('shareY',true,@(x)islogical(x)||isnumeric(x)); % common y across panels
ip.addParameter('ylim',[],@(x)isempty(x)||numel(x)==2);        % explicit override
...
case 'band'
    ...
    for k = 1:K
        ...
        box on; grid on; set(gca,'FontSize',8);
        xlabel('time (s)');
        if o.baseline
...
case {'top','tf'}
    ...
    if strcmpi(o.view,'tf')
        ...
        Y = cell(K,numel(evIdx));
        for k = 1:K
            for q = 1:numel(evIdx)
                Y{k,q} = squeeze(G(ii(sel(k)), jj(sel(k)), :, evIdx(q)))';
            end
        end
        YL = common_ylim(Y, o);
        if ~isempty(YL) && ~o.baseline && isfinite(floorV)
            YL(1) = min(YL(1), floorV - 0.02*diff(YL));
        end
...
function YL = common_ylim(Y, o)
% One y range covering every panel, so panel heights are directly comparable.
% Returns [] when the caller should let each panel autoscale.
if ~isempty(o.ylim), YL = o.ylim(:)'; return; end
if ~o.shareY,        YL = [];          return; end
v = [];
for k = 1:numel(Y)
    if isempty(Y{k}), continue; end
    y = Y{k}(:); v = [v; y(isfinite(y))]; %#ok<AGROW>
end
