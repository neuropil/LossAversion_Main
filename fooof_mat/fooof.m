...
function fooof_results = fooof(freqs, power_spectrum, f_range, settings, return_model)
    ...
    % --- Convert inputs (force 1D for FOOOF) ---
    freqs = double(freqs(:));
    power_spectrum = double(power_spectrum(:));

    freqs_py = py.numpy.array(freqs).reshape(int64(numel(freqs)));
    power_py = py.numpy.array(power_spectrum).reshape(int64(numel(power_spectrum)));

    % FOOOF expects a 2-element range; use tuple
    f_range_py = py.tuple(num2cell(double(f_range(:)')));

    % overwrite variables used downstream
    freqs = freqs_py;
    power_spectrum = power_py;
    f_range = f_range_py;

    % --- Convert settings pieces to Python-friendly types ---
    pwl = double(settings.peak_width_limits(:)');   % ensure 1x2
    peak_width_limits_py = py.tuple({pwl(1), pwl(2)});   % or py.list({pwl(1), pwl(2)})

    max_n_peaks_py     = int64(settings.max_n_peaks);
    min_peak_height_py = double(settings.min_peak_height);
    peak_threshold_py  = double(settings.peak_threshold);
    aperiodic_mode_py  = char(settings.aperiodic_mode);
    verbose_py         = py.bool(settings.verbose);

    % Initialize FOOOF object (constructor args are positional in fooof)
    fm = py.fooof.FOOOF(peak_width_limits_py, ...
        max_n_peaks_py, ...
        min_peak_height_py, ...
        peak_threshold_py, ...
        aperiodic_mode_py, ...
