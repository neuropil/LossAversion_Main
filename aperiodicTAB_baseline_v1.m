...
function [] = aperiodicTAB_baseline_v1(partID)
% partID = 'CLASE001';
...
savePath = strcat(saveCD, partID, '\Baseline');
cd(savePath)

% Get directory of names you want
mdir = dir;
dirNames = {mdir.name};
dirFilter = contains(dirNames, 'FOOOF_baseline');
...
for i = 1:length(dirFOOOFNames)

    % Load temporary file
    tempName = dirFOOOFNames(i);
    ...
    elseif matches(partID, lowLA)
        ...
        allBaselineTAB = [allBaselineTAB; tempTab];

end % for / i
% Save name
saveName = strcat(partID, '_allFOOOF.mat');

% Save
save(saveName, "allBaselineTAB");
